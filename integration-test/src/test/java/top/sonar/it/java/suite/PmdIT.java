 ...
 */
package top.sonar.it.java.suite;

import java.util.ArrayList;
import java.util.List;

import top.sonar.it.java.suite.orchestrator.PmdTestOrchestrator;
...
import static top.sonar.it.java.suite.TestUtils.keyFor;
...
class PmdIT {
    ...
    void pmdExtensions() {
        ...
        List<Issue> issues = retrieveIssues(TestUtils.keyFor(projectName, "pmd/", "Errors"));
    ...
    void testRuleAvoidDuplicateLiterals() {
        ...
        List<Issue> issues = ORCHESTRATOR.retrieveIssues(
                IssueQuery.create()
                        ...
                        .components(TestUtils.keyFor(projectName, "", "AvoidDuplicateLiterals"))
        );
    ...
    void testJunitRules() {
        ...
        List<Issue> testIssues = retrieveIssues(TestUtils.keyForTest());
        ...
        List<Issue> prodIssues = retrieveIssues(TestUtils.keyFor(projectName, "", "ProductionCode"));
    ...
    void pmdShouldHaveAccessToExternalLibrariesInItsClasspath() {
        ...
        List<Issue> issues = retrieveIssues(TestUtils.keyFor(projectName, "pmd/", "Bar"));
        assertThat(issues).hasSize(1);
    }
